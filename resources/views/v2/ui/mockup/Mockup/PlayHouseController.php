...
class PlayHouseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!empty($data['deleted_children_ids'])) {
                $deletedIds = is_array($data['deleted_children_ids']) 
                    ? $data['deleted_children_ids'] 
                    : json_decode($data['deleted_children_ids'], true);
            ...
            // Phone from step 1 = parent's phone
            $phoneNumber = $data['phone'] ?? $data['parentPhone'] ?? null;
            
            // Guardian phone from step 3 (separate guardian form)
            $guardianPhone = $data['guardianPhone'] ?? null;
            
            // First, try to find existing guardian by phone number (for returning users)
            // Only look for guardian if "Set as guardian" is checked (parent is guardian)
            $isSetAsGuardian = !empty($data['setAsGuardian']) && $data['setAsGuardian'] == '1';
            if ($isSetAsGuardian && !empty($phoneNumber)) {
            ...
            // Check if "Set as guardian" is checked - parent IS the guardian
            $isSetAsGuardian = !empty($data['setAsGuardian']) && $data['setAsGuardian'] == '1';
            
            // Save to parent_info table first (parentName -> parent_info)
            $parent = null;
            if (!empty($data['parentName'])) {
                // First, try to find existing parent by email
                $parentEmail = $data['parentEmail'] ?? null;
                if (!empty($parentEmail)) {
                    $parent = ParentInfo::where('email', $parentEmail)->first();
                }
                
                // Also try to find by phone if no parent found by email
                if (!$parent && !empty($phoneNumber)) {
                    $parent = ParentInfo::whereHas('phoneNumbers', function($query) use ($phoneNumber) {
                        $query->where('phone_number', $phoneNumber);
                    })->first();
                }
                ...
                // Save parent's phone to parent_phone pivot table
                if (!empty($phoneNumber)) {
                    $phone = PhoneNumber::where('phone_number', $phoneNumber)->first();
                    if (!$phone) {
                        $phone = PhoneNumber::create(['phone_number' => $phoneNumber]);
                    }
                    $parent->phoneNumbers()->attach($phone->id, ['is_primary' => true]);
                }
            }
            
            // Handle guardian based on "Set as guardian" checkbox
            if ($isSetAsGuardian) {
                // Parent IS the guardian - use parent's data for guardian
                if (!empty($data['parentName'])) {
                    if ($guardian) {
                        $guardian->update([
                            'first_name' => $data['parentName'] ?? '',
                    ...
                    // Save parent's phone to guardian (same person) - use same phone
                    if (!empty($phoneNumber)) {
                        $phone = PhoneNumber::where('phone_number', $phoneNumber)->first();
                        if (!$phone) {
                            $phone = PhoneNumber::create(['phone_number' => $phoneNumber]);
                ...
                // Link parent and guardian (they're the same person)
                if ($parent && $guardian) {
                    $guardian->parentInfos()->attach($parent->id);
                }
            } else {
                // Separate guardian - use guardianName, guardianLastName for guardian table
                if (!empty($data['guardianName'])) {
                    ...
                    // Save guardian's phone from the guardian form
                    if (!empty($guardianPhone)) {
                        $phone = PhoneNumber::where('phone_number', $guardianPhone)->first();
                        if (!$phone) {
                            $phone = PhoneNumber::create(['phone_number' => $guardianPhone]);
                        }
                        $guardian->phoneNumbers()->syncWithoutDetaching([$phone->id => ['is_primary' => true]]);
                    }
                ...
                } elseif ($guardian && $parent) {
                    // Guardian fields are empty but we found guardian - keep existing guardian, link to parent
                    $guardian->parentInfos()->attach($parent->id);
                }
            }

            // Create or update children (loop - can have multiple children)
            // Field names: child[0][name], child[0][birthday], child[0][playDuration], child[0][price], child[0][childId]
            $savedChildren = [];
            ...
            if (!empty($data['child']) && is_array($data['child'])) {
                foreach ($data['child'] as $childData) {
                    ...
                    // Check if this is an existing child (has childId) or new child
                    $childId = $childData['childId'] ?? null;
